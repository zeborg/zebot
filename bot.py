...
async def on_message(message):
    ...
    if message.content.lower() == f'{b_t} avatar':
        ava_embed = discord.Embed(title = f'{message.author.name}\'s Avatar', url = f'{message.author.avatar_url}', colour = random_colors())
        ava_embed.set_image(url = f'{message.author.avatar_url}')
        await message.channel.send(embed = ava_embed)
    elif message.content.startswith(f'{b_t} avatar'):
        if len(message.mentions) > 0:
            ava_embed = discord.Embed(title = f'{message.mentions[0].name}\'s Avatar', url = f'{message.mentions[0].avatar_url}', colour = random_colors())
            ava_embed.set_image(url = f'{message.mentions[0].avatar_url}')
            await message.channel.send(embed = ava_embed)
